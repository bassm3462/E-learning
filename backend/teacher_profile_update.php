...
if (isset($_POST['update'])) {
   ...
   $update_profile->bindParam(":id", $teacher_id);
   ...
   if (!empty($image)) {
      ...
      if ($image_size > 2000000) {
         ...
         $update_image->bindParam(":id", $teacher_id);
   ...
   if (!empty($previous_pass) || !empty($new_pass) || !empty($confirm_pass)) {
      if ($previous_pass != $old_pass) {
         ...
         $update_password->bindParam(":password", $new_pass);

         $update_password->bindParam(":id", $teacher_id);
...
<body>
   ...
   <h1 class="title"> update <span>teacher</span> profile </h1>

   <section class="update-profile-container">

      <?php
      $select_profile = $conn->prepare("SELECT * FROM `user_FORM` WHERE ID = :id");
      $select_profile->bindParam("id", $teacher_id);
      ...
      <form action="" method="post" enctype="multipart/form-data">
         ...
         <div class="flex">
            <div class="inputBox">
               <span>username : </span>
               <input type="text" name="name" required class="box form control" placeholder="enter your name" value="<?= $fetch_profile['name']; ?>">
               <span>email : </span>
         ...
         <div class="flex-btn">
            ...
            <a href="Techer/admin.php" class="option-btn">go back</a>
         </div>
      </form>
